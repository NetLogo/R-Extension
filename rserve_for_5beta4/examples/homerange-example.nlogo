...
to calc-homerange
  rserve:eval "library(adehabitat)"

  ;; create an empty data.frame"
  rserve:eval "turtles <- data.frame()"
  
  ;; merge the Name, X- and Y-lists of all animals to one big data.frame
  ask animals
  [
    (rserve:putdataframe "turtle" "X" X "Y" Y)     
    rserve:eval (word "turtle <- data.frame(turtle, Name = '" Name "')")
    rserve:eval "turtles <- rbind(turtles, turtle)"
  ]
   
  ;; split the data.frame into coordinates and factor variable 
  rserve:eval "xy <- turtles[,c('X','Y')]"
  rserve:eval "id <- turtles$Name" 
  ;; calculate homerange
  rserve:eval "homerange <- mcp(xy, id)"
...
to plot-into-file
  ...
  rserve:eval (word "pdf('" homerange-plot ".pdf')" )
  rserve:eval (word "plot(homerange, xlim=c(" min-pxcor "," max-pxcor "), ylim=c(" min-pycor "," max-pycor "))")
  rserve:eval "dev.off()" 

  ;; plot the homerange area depending on the homerange level (into pdf-file)
  rserve:eval (word "pdf('" homerange-level-plot ".pdf')" )  
  rserve:eval "mcp.area(xy, id, unout='m2')"  
  rserve:eval "dev.off()" 
...
to mark-homeranges
  ...
  [  
    ...
    rserve:eval (word "temp <- subset(homerange, ID=='"Name"')")
    let tempX rserve:get "temp$X"
    let tempY rserve:get "temp$Y"
...
to plot-area
  ...
  rserve:eval (word "area <- mcp.area(xy, id,  unout='m2', percent = seq(" precstart ",100, by = " precincre "), plotit=FALSE)")

  ;; plot the homerange area of every animal, using temporary plot pen
  ask animals
  [ 
    let arealist rserve:get (word "area$" Name)    
...
333
324
Note, that the path belongs to the Rserve server. Look in the working directory of the server for the output.\nFurthermore, the Rserve serve must be configured to support file operations! 
11
0.0
1
