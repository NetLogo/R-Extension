...
to init
  ; adapt port and host to your settings!
...
to create-and-write-data
  ...
  (rserve:putagentdf "ag" turtles "who" "xcor" "ycor")
...
to load-and-show-data
  ...
  let xc rserve:get "ag$xcor"
  let yc  rserve:get "ag$ycor"
...
to write-data-with-vector-dataframe
  ...
  (rserve:putagentdf "ag" turtles "who" "xcor" "ycor" "test")

  ; mark column test as class "AsIs"
  rserve:eval "ag$test <- I(ag$test)"
...
229
52
Note, that this path is used by the Rserve server! Furthermore, the Rserve serve has to support file operations! 
11
0.0
1
