...
/**
 * Class MenuBar Used to add a menubar to the JavaGD plot window with copy and save events.
 *
 * @version 1.0beta
 */
public class MenuBar extends JMenuBar implements ActionListener, ItemListener {
  ...
  String getFileDlg(String paramString) {
    ...
    if (returnVal == JFileChooser.APPROVE_OPTION) {
      ...
      if (!str.endsWith(paramString)) str = str + "." + paramString;
    }
    return str;
  }
  ...
  String escapeStr(String paramString) {
    ...
    while (i < paramString.length()) {
      ...
      if ((c == '"') || (c == '\\')) localStringBuffer.append("\\");
      ...
      if (str3 != null) {
        try {
          ...
          JOptionPane.showMessageDialog(
              null, localException, "Error in R-Extension", JOptionPane.INFORMATION_MESSAGE);
        }
      }
    }
  }
