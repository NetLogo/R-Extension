...
to setup
  ...
  crt 40
  [
    set weight rserve:get "rweibull(1,1)"
    set height rserve:get "rbeta(1,1,1)"
  ]
  reset-ticks
end
...
to go
  ...
  ask turtles
  [
    set weight weight + rserve:get "rweibull(1,1)"
    set height height + rserve:get "rweibull(1,2)" 
  ]
  ;; create R list from turtles 
  (rserve:putagent "turtles" turtles "weight" "height")
  ;; calculate correlation between weight and height
  rserve:eval "c <- cor.test(turtles$weight, turtles$height, method = 'spearm', alternative = 'g')"
  
  let rho rserve:get "c$p.value"
  let p rserve:get "c$estimate"
