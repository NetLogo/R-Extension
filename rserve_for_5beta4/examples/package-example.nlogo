...
to test-pcf1
  ...
  rserve:eval "data(simdat)"
  rserve:eval "p <- pcf(simdat)"
  rserve:eval "K <- Kest(simdat)"
  rserve:eval "p2 <- pcf(K)"
  let pcf_r (rserve:get "p2$pcf")
  let r (rserve:get "p2$r")
...
to test-pcf2
  ...
  rserve:eval "p <- pcf(agppp)"
  rserve:eval "K <- Kest(agppp)"
  rserve:eval "p2 <- pcf(K)"
  let pcf_r (rserve:get "p2$pcf")
  let r (rserve:get "p2$r")
