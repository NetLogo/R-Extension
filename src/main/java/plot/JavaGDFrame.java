...
/**
 * Class JavaGDFrame Used as custom window for JavaGD plot device.
 *
 * @version 1.0beta
 */
public class JavaGDFrame extends GDInterface implements WindowListener {
  ...
  public void gdOpen(double paramDouble1, double paramDouble2) {
    ...
    this.jfr =
        new javax.swing.JFrame("JavaGD from R-Extension") {
          private static final long serialVersionUID = 8263748858987338205L;

          public void dispose() {
            if (JavaGDFrame.this.c != null) JavaGDFrame.this.executeDevOff();
            super.dispose();
          }
        };
  ...
  public void executeDevOff() {
    if ((this.c == null) || (this.c.getDeviceNumber() < 0)) return;
    try {
      engine.parseAndEval(
          "try({ dev.set(" + (this.c.getDeviceNumber() + 1) + "); dev.off()},silent=TRUE)");
