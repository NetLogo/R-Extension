...
to setup
  ; initialize the Rserve connection
...
to sendPatchesToR
  ; send patches color to data.frame
   (rserve:putagentdf "patches" patches "pxcor" "pycor" "pcolor")
end

to createMatrix
  ; get worlds dimension and send it to R
  rserve:put "xdim" world-width 
  ; create matrix from data.frame column pcolor
  rserve:eval "m1 <- matrix(patches$pcolor, xdim)"
  ; transpose matrix 
  rserve:eval "m1 <- t(m1)"

  ; print the matrix into the Rserve shell
  rserve:eval "print(m1)"
end
@#$#@#$#@
GRAPHICS-WINDOW
210
...
160
142
output will be printed into your Rserve window (if it is running in a shell).
11
0.0
1
