 ...
 * This class only demonstrate a 'dir' or 'ls' within current (execution) path, if no parameters are
 * used. If parameters are used, the first one is the system command to execute, the others are its
 * system command parameters. <br>
 * To be system independent, an <b><a
 * href="http://www.allapplabs.com/java_design_patterns/abstract_factory_pattern.htm">Abstract
 * Factory Pattern</a></b> will be used to build the right underlying system shell in which the
 * system command will be executed.
 *
 * @author <a href="http://stackoverflow.com/users/6309/vonc">VonC</a>
 * @see <a href="http://stackoverflow.com/questions/236737#236873">How to make a system call that
 *     returns the stdout output as a string in various languages?</a>
 */
public final class SystemCall {

  /**
   * Execute a system command. <br>
   * Default is 'ls' in current directory if no parameters, or a system command (if Windows, it is
   * automatically translated to 'dir')
   *
   * @param args first element is the system command, the others are its parameters (NOT NULL)
   * @throws IllegalArgumentException if one parameters is null or empty. 'args' can be empty
   *     (default 'ls' performed then)
   */
  public static void main(final String[] args) {
    String anOutput = "";
    if (args.length == 0) {
      anOutput = Exec.execute("R --version");
    } else {
  ...
  public static final class Exec {
    ...
    public static String execute(final String aCommand, final String... someParameters) {
      String output = "";
      try {
        ExecEnvironmentFactory anExecEnvFactory =
            getExecEnvironmentFactory(aCommand, someParameters);
    ...
    private static ExecEnvironmentFactory getExecEnvironmentFactory(
        final String aCommand, final String... someParameters) {
  ...
  public abstract static class ExecEnvironmentFactory {
     ...
     * Means that a command might be translated, if it does not fit the right OS ('dir' => 'ls' on
     * unix)
     *
    ...
    protected final String buildCommandLine(
        final String aCommand, final ArrayList<String> someParameters) {
