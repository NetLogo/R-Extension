...
to calc-diversity
  ...
  (rserve:putagentdf "dfpatches" patches "pxcor" "pycor" "beechcount" "birchcount" "hornbeamcount" "oakcount" "maplecount" "ashcount" "poplarcount") 

  ;; create a data.frame without the coordinates
  rserve:eval "df <- dfpatches[,3:7]" 

  ;; load R-library vegan (must be installed!)
  rserve:eval "library(vegan)"
  
  ;; calculate diversity
  rserve:eval "spec <- specnumber(df)"
  rserve:eval "shan <- diversity(df, 'shannon')"
  ;; Pielou's evenness (J), if spec = 0, value of pielou will be NaN:
  rserve:eval "pielou <- shan/log(spec)"
  rserve:eval "simp <- diversity(df, 'simpson')"
  ...
  ;; to get just the diversity values as a list, call: print rserve:get "shan"
  

  ;; create lists with the coordinates
  let xlist rserve:get "dfpatches$pxcor"
  let ylist rserve:get "dfpatches$pycor"   
  ...
  [
    ...
    [
      set speccount (rserve:get (word "spec[" counter "]"))
      set shannon (rserve:get (word "shan[" counter "]"))
      set simpson (rserve:get (word "simp[" counter "]"))
      set pielou (rserve:get (word "pielou[" counter "]"))    
    ]
